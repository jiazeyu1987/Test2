...
bool qSlicerMainWindowPrivate::confirmCloseApplication()
{
  Q_Q(qSlicerMainWindow);
  if (q->need_confirm) {
      QMessageBox* messageBox = new QMessageBox(QMessageBox::Warning, QStringLiteral("确认退出吗"), QStringLiteral("退出将损失当前的进度"), QMessageBox::NoButton, q);
...
void qSlicerMainWindow::showEvent(QShowEvent *event)
{
  ...
  //d->PythonConsoleDockWidget->hide();
}
...
{
  Q_D(qSlicerMainWindow);
  qSlicerApplication::application()->mrmlScene()->InvokeEvent(1331229, nullptr);
