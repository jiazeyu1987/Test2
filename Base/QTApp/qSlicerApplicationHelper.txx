    ...
    )
{
  ...
  bool showSplashScreen = qJMainWindow::ProjectSettings->value("General/show_splash_screen") == "2";
...
// See details at https://issues.slicer.org/view.php?id=4252
  ...
  if (showSplashScreen)
    {
    auto path = qJMainWindow::GetProjectBasePath() + "/Resources/Icons/Slicer-SplashScreen.png";
    QPixmap pixmap(path);
    ...
    splashMessage(splashScreen, QStringLiteral("初始化..."));
    splashScreen->show();
    }
  ...
  splashMessage(splashScreen, QStringLiteral("注册模块..."));
  ...
  splashMessage(splashScreen, QStringLiteral("初始化模块..."));
  ...
  // Create main window
  splashMessage(splashScreen, QStringLiteral("初始化用户界面..."));
  if (enableMainWindow)
    {
  ...
  foreach(const QString& name, moduleFactoryManager->instantiatedModuleNames())
    ...
    splashMessage(splashScreen, QStringLiteral("加载模块 \"") + name + "\"...");
