...
void qSlicerWebDownloadWidget::handleDownload(QWebEngineDownloadItem *download)
{
  ...
  QPushButton *loadButton = messageBox->addButton(QStringLiteral("加载"), QMessageBox::AcceptRole);
  messageBox->setDefaultButton(loadButton);
  loadButton->setToolTip(tr("Download the data and open in Slicer"));
  QPushButton *saveButton = messageBox->addButton(QStringLiteral("保存"), QMessageBox::AcceptRole);
  saveButton->setToolTip(tr("Save to an arbitrary location"));
  QPushButton *abortlButton = messageBox->addButton(QStringLiteral("取消"), QMessageBox::AcceptRole);
