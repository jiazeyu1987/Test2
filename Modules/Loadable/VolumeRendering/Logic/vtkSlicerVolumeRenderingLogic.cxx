...
void vtkSlicerVolumeRenderingLogic::setVolumeRendering(vtkMRMLScalarVolumeNode* node, bool b) {
    if (!node) {
        return;
    }
    if (true) {
        vtkMRMLVolumeRenderingDisplayNode* displayNode = this->GetFirstVolumeRenderingDisplayNode(node);
        auto disp = node->GetDisplayNode();
        if (!disp) {
            node->CreateDefaultDisplayNodes();
            disp = node->GetDisplayNode();
        }
        if (!displayNode) {
            displayNode = CreateVolumeRenderingDisplayNode();
            displayNode->SetVisibility(0);
            GetMRMLScene()->AddNode(displayNode);
            ...
            UpdateDisplayNodeFromVolumeNode(displayNode, node);
            displayNode->SetIgnoreVolumeDisplayNodeThreshold(0);
            displayNode->EndModify(wasModifying);
        }
        auto node1 = this->GetPresetByName("MR-Default");
        if (displayNode->GetVolumePropertyNode() == nullptr)
        {
        ...
        displayNode->GetVolumePropertyNode()->Copy(node1);
